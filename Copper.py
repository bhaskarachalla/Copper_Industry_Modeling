...
selected = option_menu(None, ['HOME',"PRICE PREDICTION","STATUS PREDICTION","CONCLUSION"],
            icons=["chart_with_upwards_trend"],orientation='horizontal',default_index=0)
...
if selected=='HOME':
    ...
    with b1:
        ...
        st.write('* ML Regression model which predicts continuous variable :skyblue[**‘Selling_Price’**].')
    ...
    with b2:
        ...
        st.write('#### CLASSIFICATION - ***:violet[Random Forest Classification]***')
...
if selected=='PRICE PREDICTION':
    try:
        if 'PRICE PREDICTION':
            ...
            st.write(
                    '##### ***<span style="color:white">Fill all the fields and Press the below button to view the :red[Pedicted price]   of copper</span>***',
...
if selected=='STATUS PREDICTION':
    try:
        if 'STATUS PREDICTION':
            ...
            st.write('##### ***<span style="color:white">Fill all the fields and Press the below button to view the status :violet[WON] / :red[LOST] of copper in the desired time range</span>***',
                        unsafe_allow_html=True)
...
if selected == 'CONCLUSION': 
    st.markdown("## :blue[Synthesis]")  

    st.write('## Copper Price and Status Determinants :')
    st.write(" :green[## Supply and Demand:] The balance between supply and demand for copper plays a significant role in determining its price. Factors such as industrial production, construction activity, and global economic growth can affect demand for copper. Similarly, disruptions in copper mining operations, changes in mining regulations, or geopolitical tensions in major copper-producing regions can impact the supply of copper.")     
    st.write(':green[## Growth in technology:] The growing :red[**trend of electric vehicles (EVs)**] and renewable energy technologies, such as :red[**solar**] and :red[**wind**], can influence copper demand due to its use in wiring and components.')      
    st.write(':green[## Currency Fluctuations:] Since copper is traded globally, fluctuations in currency exchange rates can influence its price. A weaker currency relative to the U.S. dollar, in which copper is often priced, can lead to higher prices for copper in local currency terms.')
